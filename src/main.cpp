...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
    ...
    if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
        ...
        if (strcmp(action, "toggle5") == 0) {
            ...
            WiFiClient client;
            // HTTPClient http;        
            // IPAddress server (172,16,15,100);
            // byte server[] = { 172, 16, 15, 100 }; // Google
            const char* server = "172.16.15.100";

            if (client.connect(server, 80)) {
                Serial.println("connected");
                client.print("GET /api/playlist/Start/start"); // This// Make a HTTP request:
                client.println();
            } 
